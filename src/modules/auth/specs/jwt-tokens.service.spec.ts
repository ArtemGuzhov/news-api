...
describe('JwtTokensService', () => {
    ...
    describe('refreshTokens', () => {
        ...
        it('throw ACCESS_DENIED error if user refresh token to equal null', async () => {
            ...
            jest.spyOn(service['_usersService'], 'findOne').mockResolvedValue({ ...userEntityMock, refreshToken: null })

            expect(service.refreshTokens(id, refreshToken)).rejects.toThrow('ACCESS_DENIED')
            expect(service['_usersService'].findOne).toHaveBeenCalledTimes(1)
        })
    ...
    describe('updateRefreshTokenHash', () => {
        it('should update refresh token hash', async () => {
            ...
            expect(service['_usersService'].update).toHaveBeenCalledTimes(1)
        })
    })
